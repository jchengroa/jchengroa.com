...
export default function CommonTab({ 
    ...
    const isCommonChanged = JSON.stringify(commonData) !== JSON.stringify(savedCommonData);
    ...
    return (
        <div className="space-y-6 sm:space-y-8">
            <div className="p-4 sm:p-6 sm:p-7 rounded-2xl sm:rounded-3xl bg-white dark:bg-gray-900 border border-gray-200/80 dark:border-gray-800 shadow-sm space-y-5 sm:space-y-6">
                <SectionCardHeader
                    title="Common UI Dialogues & Button Prompts"
                    keyBadge="key: common"
                    description="Shared labels, feedback dialogues, button prompts, and global microcopy."
                    hasUnsaved={isCommonChanged}
                    onUpload={() => onSaveKey && onSaveKey('common', commonData)}
                    isSaving={saving}
                    customAction={
                        <button
                            type="button"
                            onClick={() => setShowAddForm(!showAddForm)}
                            className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 hover:bg-blue-100 transition-colors cursor-pointer shrink-0"
                        >
                ...
                {/* Add Custom Key Form */}
                {showAddForm && (
                    <form onSubmit={handleAddCustomKey} className="p-4 rounded-xl sm:rounded-2xl bg-blue-50/50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/50 space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <input
                                ...
                                className="px-3.5 py-2.5 sm:py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm sm:text-xs font-mono text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-600"
                            />
                            <input
                                type="text"
                                required
                                ...
                                className="px-3.5 py-2.5 sm:py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm sm:text-xs font-semibold text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-600"
                            />
                        </div>
                        <button
                            type="submit"
                            className="px-4 py-2 sm:py-1.5 rounded-xl text-xs font-black text-white bg-blue-600 hover:bg-blue-700 shadow-sm cursor-pointer"
                        >
                            Save Property
                        </button>
                    </form>
                ...
                {customKeys.length > 0 && (
                    <div className="pt-4 border-t border-gray-100 dark:border-gray-800 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                            {customKeys.map(k => (
                                <div key={k} className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-gray-50 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-700 space-y-2">
                                    <div className="flex items-center justify-between">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-3 py-2 sm:py-1.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm sm:text-xs font-semibold text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-600"
                                    />
                                </div>
                            ))}
                        </div>
